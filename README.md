...
The optional `conceptualize` stage maintains a minimal planning index and writes focused Slices only when useful. The `implementation-plan` orchestrator dispatches a fresh delegated planner; that planner writes `.tasks/<feature>/SPEC.md`, the lightweight `tasks.json` registry, package Markdown, declared proof paths, and package verification report paths. `SPEC.md` records delegated-planner provenance, `tasks.json` is bookkeeping only, and package Markdown owns work-package scope, Slice assignments, dependencies, verification expectations, proof paths, and report paths.
