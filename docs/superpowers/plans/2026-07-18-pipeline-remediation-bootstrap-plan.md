...
for durable cross-system handoff.
...
4. Consolidate planning plus cold plan challenge into one user acceptance phase by removing blocking Gate 1 before
   read-only review; Gate 2 remains the exact accepted-state authority.
5. Replace universal final `review-code` plus final `audit` duplication with one final-verification phase carrying
   distinct completeness and code-risk lenses; use a second independent agent only when named risk requires it.
6. Return final findings once to the Delivery Owner, classify and batch eligible repairs once, and forbid child-owned
   fix/verification loops.
7. After repair, rerun only affected seams and evidence unless impact is material, shared, sensitive, or uncertain;
   the Phase 1 same-cluster second-failure circuit remains authoritative.
8. Do not re-request approval for repairs and bounded reruns already inside the accepted Execution Contract;
   product/scope/risk changes and protected side effects still require explicit authority.
9. Keep the sidecar and accepted `SPEC.md` mandatory while making independent Slices conditional when the sidecar
   Accepted Brief is sufficient and no durable Slice adds independent product/design authority.
10. Reduce duplicate closure prose while retaining accepted-outcome-to-evidence traceability.
11. Establish clean-path call budgets and report planned versus actual agent amplification in existing execution
    summaries, without adding a ledger, dashboard, or receipt.

## Explicit non-goals

- Do not remove, bypass, or replace the artifact sidecar; it remains the portable durable authority for every
  planned feature.
- Do not eliminate Slices when they carry independently useful product/design authority.
- Do not weaken Gate 2, exact artifact commits, proof/evidence bindings, or final freeze semantics.
...
- Preserve root-worktree safety, bounded commands, sidecar authority, and exact freeze semantics.

## Phase 2 target metrics

- Narrow clean feature: approximately 3–4 total delegated calls from planning through final verification.
- Clean four-package feature: no more than approximately 7 total delegated calls unless named risk justifies
  package verifiers or specialists.
