...
Load after the `worktree` skill at implement Steps 4/7. Worktree references own lifecycle safety and command runbooks; this file owns implement-only sequencing: proof gates before merges, conflict handling, and status output.
...
- This file — when implement calls those runbooks and which package proof/review gates must pass first.
...
`task/<feature>/<WP-ID>` keeps the compatibility `task/` prefix; `<WP-ID>` is a work package ID, not a task ID.
...
- Merge each package branch at most once, after report/proof prechecks pass.
...
- Never merge into or push `<target-ref>`/`main` without explicit approval for that exact target. Feature push, tests, or “looks good” is not target approval.
...
4. Package agents may commit per task ID; the orchestrator merges the package branch once.
...
1. Validate report, `SELF_REVIEW`, proof entries, verification commands, and command-safety screening via `integration-checkpoint.md`.
2. Confirm ignored `.tasks` proof artifacts were not force-added or committed; hand proofs through the task store, not package branch merges.
3. Merge one package branch using `feature-package-workflow.md`.
4. Return to `integration-checkpoint.md` for integrated proof validation, targeted package review, proof acceptance, and task-completion gates.

Do not mark tasks done merely because the package branch merged.
...
3. For substantive logic/API/schema/design conflicts, run `git merge --abort`, block the package with `blocked_reason: "merge conflict with <other-package> in <file(s)>"`, or delegate in-scope repair when no design decision is needed.
...
```text
Integrated packages: ✅ WP1 merged; ⏸ WP2 kept for downstream review/fix
