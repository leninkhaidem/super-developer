...
- Package repair/verification agents: `plugins/super-developer/skills/implement/references/repair-agent-contract.md`

The orchestrator should load `delegation-dispatch.md` only. It should pass the appropriate role-specific contract path to each sub-agent and should not load `package-agent-contract.md`, `repair-agent-contract.md`, or `${SUPER_DEVELOPER_PLUGIN_ROOT}/references/clean-code-rules.md` into main context by default.
