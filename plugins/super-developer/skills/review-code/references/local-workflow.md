...
For review-owned fixes, include the finite repair-time allowance in the existing proposed fix action after loading
bounded-work policy through the parent. Preserve consumed effort across explicit follow-ups; accepting `fix` does not
reset it. Caller-owned review inherits the caller's budget/deadline and returns proposals without extending either.
