...
- Use semantic batching when a diff exceeds about 2,000 lines, spans too many files, mixes domains, includes generated churn, or is too broad for one coherent review. Split by package/module/seam/risk surface, not by arbitrary line chunks.
- Keep source and tests that prove the same behavior together when practical. Group low-risk generated, docs, snapshots, and repetitive fixtures with their owning surface; verify provenance or sample only when full review adds little confidence.
- For each batch, preserve mode context and reviewed-state metadata, run the bounded reviewer topology, assign stable dedupe keys, and carry confirmed/disputed/downgraded/suggestion findings into a cross-batch dedupe set.
- After batches, run one global integration pass over the consolidated findings and whole reviewed state to detect duplicates, recommendation conflicts, cross-batch serious risks, and seam issues. Do not reopen reviewer fanout unless batch boundaries cannot preserve confidence.
...
Serious candidates require Skeptic verification before reporting or fixing. Skeptic tries to disprove the finding against: introduced-by-reviewed-change, surrounding code/caller handling, framework/library absorption, reachability, documented intent, test-only scope, reviewed-change scope, and planned-feature artifact evidence. Output needs verdict, decisive disproof check(s), evidence, and reason; verbose checklist rows are optional.

Documented intent can dispute non-security/privacy/safety findings only; real security/privacy/safety risks remain confirmed with the intent noted. Test-only scope disputes serious findings unless it masks a production regression. Planned-requirement claims require concrete artifact evidence; review-code flags consistency/evidence signals only and must not become final audit.

Verdicts: `CONFIRMED`, `DISPUTED`, `DOWNGRADED`. Only confirmed 🔴/🟠 findings are reportable as serious or fixable; disputed findings are excluded; downgraded findings may appear only as actionable 🟡 suggestions.

Suggestions are report-only by default and never start a separate fix loop. Auto-fix a suggestion only when bundled with an approved 🔴/🟠 fix, same file/symbol/root cause, behavior-preserving, no public API/schema/config/permission/persistence/error/test-contract/user-visible change, no extra review surface, and optional to closing the serious finding. Cap bundled suggestions at 1–3 per batch unless the user explicitly expands scope.
...
Render one structured Markdown body, not inline diff comments: header, optional verdict line, finding counts, files/line counts, mode metadata, then `### 🔴 Blockers`, `### 🟠 Critical Issues`, and `### 🟡 Suggestions _(non-blocking)_` sections. Each finding includes title, `Path`, independently verifiable evidence, and concrete recommendation or alternatives with tradeoffs. Omit empty sections; if all are empty, render `No issues found. ✅`.

Footer: `_Review generated via bounded multi-agent analysis. Reported blockers and critical issues were independently verified by the Skeptic Agent._` plus any mode footer. Never render coverage rows, raw tags, dedupe keys, state fields, lifecycle fields, or tracking IDs. Pipeline findings must be consistency/evidence signals and must not claim exhaustive Slice/package/proof completion.
...
Required closure tuple per assigned dedupe key: `dedupe_key`, `verdict: closed|partially_closed|not_closed|reopened`, evidence, remaining risk, and `next_action: none|same_scope_fix|widened_verification|full_rereview|authority_boundary`. Also include regression sniff for affected security/privacy/safety/data/public-contract/concurrency/performance surfaces, widening triggers, and readiness: `ready_for_audit`, `needs_fix`, `needs_widened_review`, or `needs_user_authority`.

Named widening triggers: `scope_expansion`, `public_api_or_schema_change`, `sensitive_risk_surface`, `cross_package_impact`, `proof_invalidation`, `large_delta`, `non_closed_verdict`. Report new issues from this phase only when they are serious regressions introduced by the fix or needed to explain a concrete widening trigger.
