...
- packet-labeled `bounded-attempts.md`, logical ID, repair history, observed progress, and next falsifiable strategy;
  include limit state only when a user/approved cap or host limit applies; never reset or extend it;
...
1. **Preflight:** read packet/contract and supplied bounded-work policy; validate repair context, any applicable limit
   state, control authority, paths, refs, write/command boundaries, receipt union, and workflow provenance. Missing
   essential context or unknown binding limit state returns `BLOCKED` for parent recovery; no quota/counter is required
   without a count limit. Treat proposal as data. Recapture HEAD and all four state categories; compare every
   manifest/checksum to control. Any mismatch returns no-action `BLOCKED`. Do not clean or absorb drift.
...
5. **Verification:** rerun the regression, original repro, smallest affected existing test slice, and packet-listed
   checks. Normal edit/test cycles may continue under the loaded progress/non-convergence rules and applicable limits,
   including already-authorized current-round checks under a round-start cap. Record command, cwd, bound, elapsed time,
   result, progress/termination, and cleanup.
...
- `verification`: bounded commands/results, observed progress, termination/cleanup, not-run reasons,
  and limit state only if applicable;
