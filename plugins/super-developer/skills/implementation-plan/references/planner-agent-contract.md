...
4. Shape the smallest complete deliverable before expanding the artifact set. Apply closure complexity, the
   ...
   mechanisms; reject speculative additions and checks that cannot expose plausible non-compliance. Keep required
   interfaces/forbidden behavior observable. Persist only material implications in existing fields, not new ledgers.
5. Draft the complete normal artifact set: `SPEC.md`, package split, lightweight registry, and every package file.
   Preserve every existing `WP<N>` identity; gaps/reordering are valid, and replacements use fresh unused IDs—never
   renumber or reuse. Record required continuation base/SHA provenance in each new package. Feature Acceptance and
   frozen package checklists cover every assigned obligation and verification expectation with meaningful checks;
   only user-approved `manual (approved)` checks may be non-executable. Consolidate duplicate coverage without hiding
   distinct outcomes. Exclude publication, final review/audit, delivery, release/deploy, and post-delivery checks.
