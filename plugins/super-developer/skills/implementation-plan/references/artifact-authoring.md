...
- run helper `scan` for package worktree targets using local configs only, with raw output `.tasks/<feature>/semgrep/<WP-ID>.semgrep.json` and summary output `.tasks/<feature>/semgrep/<WP-ID>.semgrep-summary.json`;
