...
- Planner provenance is a file-only process receipt in `SPEC.md`; missing, unknown, or contradicted delegated-planner provenance is a blocking process defect before Gate 2 approval.
- Do not use hidden chat history, runtime identity claims, or arbitrary `tasks.json` fields as planner provenance.
...
1. Resolve `.tasks/<feature>/`; require `SPEC.md`, `tasks.json`, declared package Markdown paths, proof paths, report paths, safe Slice inventory paths when Slices exist, and `SPEC.md ## Planner Provenance` with delegated invocation status, planner contract path, authority packet/source summary, and validation command/result.
...
3. Read only enough registry/SPEC metadata to present Gate 1 artifact paths, package IDs/dependencies, Slice inventory paths, proof/report paths, planner provenance status, known flags, and out-of-scope items; do not ingest package or Slice semantics for review.
...
8. Present Gate 2 with final deliverables, planner provenance status, reviewers run, escalation decisions, auto-refinements, deferrals, dismissals, parallel/serial rationale, proof/report expectations, and remaining risks.
...
- `SPEC.md ## Planner Provenance` is missing, unknown, contradicted, dependent on hidden chat/runtime identity, moved into `tasks.json`, or inconsistent with the delegated planner contract/authority-packet model.
...
Return Gate 2 status, planner provenance status, reviewers run, findings and resolutions, changed artifact paths, validation command results, package dependency and parallel/serial rationale, approved deferrals, unresolved blockers, and the next recommended stage.
