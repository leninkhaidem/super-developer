...
- **Interface contract — when interface-bearing** — if a reasonable implementation could satisfy the words and still be wrong (wrong command, API, flag, path, or output), capture an inline interface contract per the parent-loaded Slice-authority contract, including its mandatory forbidden behaviors.
