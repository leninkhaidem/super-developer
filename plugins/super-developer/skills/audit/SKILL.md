...
Strict verification that all planned feature requirements and acceptance criteria are complete in the current codebase. Checks SPEC.md, tasks.json, package proof files, final code state, and Development Quality Contract MUST-level compliance — this is not a full code review.
...
First, read SPEC.md, tasks.json, and every `.tasks/<feature>/proofs/*.proof.json` file. Package proof evidence is required for every proof-only planned-feature audit, including standalone invocation against `.tasks/<feature>/`.
...
- If a SPEC item cannot be verified programmatically, require durable user-approved manual evidence in the relevant package proof file; otherwise flag `[SPEC]` and fail.
...
- If package proof evidence is missing, malformed, stale, failed, blocked, wrong-package, wrong-targeted-review-package, or has unapproved `manual_required` evidence for the criterion — flag `[ISSUE]` and fail.
...
Apply this section for every audit. Missing package proof files are a [PROOF] failure for proof-only planned features.

- Treat `.tasks/<feature>/proofs/*.proof.json` files as the durable criterion evidence source.
- For each completed task acceptance criterion, confirm a package proof entry references the correct criterion ID, task ID, package ID, source refs, context bundles, state/commit/worktree, files or symbols, commands/results when applicable, observed result, edge cases when relevant, and mock disclosure.
- Fail stale evidence when cited files, symbols, commands, or package outputs changed after the recorded proof state; use `taskctl validate-proofs --worktree <audit-worktree>` output as the mandatory stale-evidence baseline.
...
8. [PROOF] <description> — package proof missing, malformed, stale, blocked/failed, wrong-package, wrong-targeted-review-package, or unapproved manual evidence
