...
Run the final non-bypass Slice-first completion gate from files and the top integrated code state, including bounded stack-aware artifact sets when supplied.
...
- Package final readiness must be mechanically valid, matrix-clean, and bound to the resolved integrated state before audit dispatch.
...
2. For every artifact set, require `SPEC.md`, `tasks.json`, `packages/`, `proofs/`, `reports/`, and authoritative Slice paths; stop on missing, unsafe, unreadable, or omitted base-feature deliverables in the top branch.
3. Resolve the top audit worktree and fail when uncertain:
   ...
   - record absolute artifact roots, absolute worktree root, git ref, commit, base/target refs when known, feature/stack slug, and included artifact sets.
4. Load `../../references/tool-usage.md`, then run the read-only final mechanical gate once per artifact root/task set:
...
9. Load `references/audit-subagent-contract.md` and dispatch a cold read-only auditor through a fresh sub-agent/role invocation with an explicit packet: top worktree/code state, artifact set list, git metadata, SPEC/registry/package/proof/report/Slice paths, package-completion and `validate-final` results per set, review-code paths or `none`, and Semgrep expectations.
...
- `sliceproof.py validate-final` fails for any resolved artifact root and registry.
