...
Normal cleanup binds canonical path, HEAD/complete state, full direct ref/SHA, landing/base state when required, and
remote expected state only for separately authorized remote actions. Recapture immediately. Direct-ref deletion uses
`git update-ref --no-deref -d <full-ref> <old-sha>` CAS after normal worktree removal. Never force.

## Receipt-Bound Probe Boundary

A probe authorized by auto-resolve or exact current-task approval may clean only through its creation receipt and
`probe-cleanup.md`: classify NUL deltas, restore/remove exact owned state, and prove full ref/expected base/index
digest/status/process/data closure before normal removal/direct full-ref CAS. Verify no tracking config and
`remote_action=none`; perform no network/credential check or remote mutation. A coincidental remote ref stays
untouched. Any unowned/uncertain delta preserves the probe and stops. Continuation package worktrees/refs never use
receipt-bound probe cleanup and remain through final gates.

## Package Cleanup — Whole Feature Only

No active or retired package cleanup occurs before all packages, final integrated review/audit, every source gate
actually due under the approved policy, and contracted later delivery gates pass. `local-only` or not-yet-due
publication creates no remote prerequisite. Planned hotfix applies the same tip eligibility at its separate final
cleanup gate against final hotfix integration HEAD.
...
if git -C "$LANDING" merge-base --is-ancestor "$TIP" "$FINAL"; then :
  ...
  printf 'preserve: unique unmerged package commits ref=%s tip=%s base=%s final=%s\n' \
    "$REF" "$TIP" "$BASE" "$FINAL" >&2
  exit 1
fi
...
For delivery context `feature`, use only the source-publication packet. Preserve a local recovery commit/ref for every
package. Non-due/`local-only` source gates run no network; due remote gates stop on any network/credential/push/SHA
verification failure. Planned hotfix has no feature ref and uses only its separately contracted `hotfix/<name>` gate.

Sidecar publication is independently authorized and runs only from its artifact worktree:
...
Neither source nor sidecar publication approves the other, target delivery, or cleanup. Never merge artifact refs into
code history.
...
Status 1 alone means merge needed. A clean approved non-root target checkout may substitute. Locked local target stays
unchanged; report only the integration result, then request target-push approval.
...
Use only for delivery context `feature`. Recapture current delivery landing state before removing feature worktree/ref;
stale approved SHA is insufficient:
...
Bugfix/hotfix additionally recapture landing HEAD/state and prove repair SHA ancestry before removal:
...
local-only under its receipt; package cleanup occurs only at the final whole-feature gate above.
