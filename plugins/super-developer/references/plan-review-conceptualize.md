...
7. Require the selected index and every Slice needed for coverage or assignment review to exist and be readable before review can pass. Missing, unsafe, or unreadable paths are `BLOCKER` findings; duplicated paths are also `BLOCKER` findings because implementation would otherwise receive unverified background context.
