...
| **review-code** | Bounded multi-agent code review. Always runs one **Code Reviewer**, adds at most one optional **Specialist Reviewer** for the highest-priority risk trigger, and uses a **Skeptic Agent** to verify serious findings before reporting. When paired plan artifacts are expected or discoverable, missing/mislocated/stale/invalid/unreadable/partial `SPEC.md` + `tasks.json` context is a serious incomplete-plan-context finding; when both are fully usable, the Code Reviewer performs strict task-compliance against all planned requirements, tasks, and acceptance criteria. Standalone PR/local reviews without expected or discoverable plan artifacts proceed normally. Audit remains the authoritative completeness gate. Local `fix` delegates non-trivial fixes to a Fix Implementer and verifies the fix delta with a Fix Reviewer; PR mode is review-only and has no code-fix path. | Pipeline + Standalone + PR review |
...
- **Task-compliance:** usable paired `SPEC.md` and `tasks.json`, plus audit results when present, require the default Code Reviewer to check reviewed code against all planned requirements, tasks, and acceptance criteria. When plan artifacts are expected or discoverable, missing, mislocated, stale, invalid, unreadable, truncated, paginated, or partially read paired context — including both artifacts being absent — is itself a serious finding and blocks clean/approval language. Standalone PR/local reviews with no expected or discoverable plan artifacts proceed without task-compliance context. Visible omissions, contradictions, regressions, or unsupported completion claims are review-code findings; audit remains authoritative for final PASS/FAIL completeness.
...
| Audit remains authoritative | Pipeline audit verifies "did we build what we planned" before code review begins. Review-code performs strict task-compliance when paired plan artifacts are usable; expected/discoverable but missing, mislocated, stale, invalid, or unreadable paired artifacts are review-code findings, while standalone PR/local reviews with no expected/discoverable plan artifacts proceed without that context. Audit remains the final PASS/FAIL completeness gate. |
