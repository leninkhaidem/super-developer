...
| **audit** | Post-implementation verification. Spawns a read-only sub-agent that checks every acceptance criterion against the actual codebase. Produces a PASS/FAIL report. Runs in the pipeline when selected; also invocable standalone. | Pipeline + Standalone |
| **review-code** | Bounded multi-agent code review. Always runs one **Code Reviewer**, adds at most one optional **Specialist Reviewer** for the highest-priority risk trigger, and uses a **Skeptic Agent** to verify serious findings before reporting. It uses plan/audit artifacts as context only; audit remains authoritative for full SPEC/tasks completeness. If audit is missing or skipped, review-code must not claim planned completeness. Local `fix` delegates non-trivial fixes to a Fix Implementer and verifies the fix delta with a Fix Reviewer; PR mode is review-only and has no code-fix path. | Pipeline + Standalone + PR review |
...
- **Task-awareness:** available `SPEC.md`, `tasks.json`, and audit results help review-code flag clear contradictions, regressions, audit failures, or stale audit evidence. Review-code does not re-audit every acceptance criterion; if audit is missing or skipped, it must not claim planned completeness.
...
| Audit remains authoritative | Pipeline audit verifies "did we build what we planned" when the user chooses that path. Review-code may use plan and audit artifacts as context, but it does not re-audit every acceptance criterion and must not claim planned completeness when audit is missing or skipped. |
