...
class GuardrailDocumentationRegressionTests(unittest.TestCase):
    ...
    def test_release_skill_uses_single_release_contract_gate(self) -> None:
        ...
        self.assertIn("cleanup-safety.md", release_skill)
        self.assertIn("Remove local worktree(s)", release_skill)
        self.assertIn("feature branch is merged into the target ref", release_skill)
        self.assertIn("target-merge/empty feature worktree directories last", release_skill)
        self.assertIn("git branch -d", release_skill)
